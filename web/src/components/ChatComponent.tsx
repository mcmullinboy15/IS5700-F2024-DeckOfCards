...
const DEFAULT_CHANNELS: Channel[] = [
	{ id: 'public', name: 'Public', type: 'public', description: 'General chat for all users' },
	{ id: 'poker', name: 'Poker', type: 'game', gameId: 'poker', description: 'Chat for poker players' },
	{ id: 'blackjack', name: 'Blackjack', type: 'game', gameId: 'blackjack', description: 'Chat for blackjack players' }
];
...
const ChatComponent = () => {
	...
	const [currentChannel, setCurrentChannel] = useState<Channel>(DEFAULT_CHANNELS[0]);
	...
	const [subscribedChannels, setSubscribedChannels] = useState<Set<string>>(new Set(['public']));
				...
				setMessagesByChannel(prev => {
					...
					channelMessages.forEach(newMsg => {
						if (newMsg.isReactionUpdate) {
							...
							const existingIndex = existingMessages.findIndex(msg => msg.id === newMsg.id);
							if (existingIndex !== -1) {
		...
		DEFAULT_CHANNELS.forEach((channel, index) => {
			...
			if (Array.isArray(channelMessages)) {
				...
				if (channel.id !== currentChannel.id) {
					const newMessages = channelMessages.filter(msg => 
						!messagesByChannel[channel.id]?.some(existing => existing.id === msg.id)
					).length;
	...
	const handleReaction = (messageId: string, reactionType: Reaction['type']) => {
		...
		const updatedReactions = JSON.parse(JSON.stringify(messageToUpdate.reactions || {}));
		...
		const channelIndex = DEFAULT_CHANNELS.findIndex(c => c.id === currentChannel.id);
		if (channelIndex !== -1) {
	...
	const handleReply = (messageId: string) => {
		...
		channelSubscriptions[DEFAULT_CHANNELS.indexOf(currentChannel)].sendMessage(messageData);
		setNewMessage("");
	};
	...
	const updateSettings = (category: keyof ChatSettings, setting: string, value: any) => {
		setSettings(prev => ({
			...prev,
			[category]: {
				...prev[category],
			...
			DEFAULT_CHANNELS.forEach((channel, index) => {
				...
				if (Array.isArray(channelMessages) && channelMessages.length > 0) {
					...
					if (
						...
						!settings.notifications.mutedChannels.includes(lastMessage.channelId) &&
						!settings.notifications.mutedUsers.includes(lastMessage.sender)
					) {
						// Show notification
						if (settings.notifications.desktop && notificationPermission === 'granted') {
	...
	return (
		<div 
			className={`chat-widget ${isMinimized ? 'minimized' : ''} ${isResizing ? 'resizing' : ''}`}
			style={{ 
			...
			{!isMinimized && (
				<div className="chat-container">
								...
								<div key={channel.id} className="channel-item">
									...
									{channel.type !== 'public' && (
										<button
											...
											title={subscribedChannels.has(channel.id) ? 
					...
					<div className="main-chat">
								...
								<div key={msg.id} className="message">
									{msg.replyTo && (
										<div className="reply-indicator">
											↳ Reply to: {currentMessages.find(m => m.id === msg.replyTo)?.text.substring(0, 30)}...
										</div>
									)}
						...
						<div className={`settings-container ${showSettings ? 'visible' : ''}`}>
							...
							<div className="settings-content">
								<div className="settings-tabs">
									...
									<button 
										className={activeSettingsTab === 'notifications' ? 'active' : ''}
									...
									{activeSettingsTab === 'appearance' && (
										<div className="settings-section">
											...
											<select 
												...
												onChange={(e) => updateSettings('appearance', 'emojiStyle', e.target.value)}
											>
											...
											<select 
												...
												onChange={(e) => updateSettings('appearance', 'timestampFormat', e.target.value)}
											>
													...
													{settings.notifications.sound && (
														<div className="setting-item">
															...
															<input 
																...
																onChange={(e) => updateSettings('notifications', 'soundVolume', Number(e.target.value))}
															/>
															<button 
